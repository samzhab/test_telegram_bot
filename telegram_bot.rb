...
module InvoiceUtils
  def self.setup_worldwide_invoice_details
    config = YAML.safe_load(ERB.new(File.read('invoice_details.yml')).result)
  ...
  def self.dispatch_invoice(bot, chat_id, invoice_details)
    ...
    puts "invoice details ------------- #{invoice_details}"
  end
end
...
class MyTelegramBot
  ...
  def handle_precheckout_query(bot, pre_checkout_query)
    puts "pre_checkout_query --------------------------------- #{pre_checkout_query}"
  ...
  def display_ethiopia_payment_options(bot, chat_id, checkout_url, _payload)
    ...
    message_text = format(UI_STRINGS['ethiopia_payment_options']['message_text'], checkout_url: checkout_url)
    complete_payment_button_text = UI_STRINGS['ethiopia_payment_options']['complete_payment_button']

    keyboard = [Telegram::Bot::Types::InlineKeyboardButton.new(text: complete_payment_button_text,
                                                               callback_data: 'complete_payment', url: checkout_url)]
